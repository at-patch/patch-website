...
import { daysLate, isOrderTab, OVERDUE_AFTER_DAYS, type OrderTab, type OrderTabCounts } from "@/lib/order-buckets";
...
export default function OrdersTabTable({ limit = 10 }: { limit?: number }) {
  ...
  // Read the tab off the URL after mount rather than during render: the dashboard
  // is a server component, and reading search params during render would force it
  // to re-run on every tab click.
  useEffect(() => {
    const fromUrl = new URLSearchParams(window.location.search).get("tab");
    // eslint-disable-next-line react-hooks/set-state-in-effect -- window is unavailable during SSR, so the URL can only be read after mount
    if (fromUrl && isOrderTab(fromUrl)) setTab(fromUrl);
  }, []);

  const load = useCallback(
    async (nextTab: OrderTab) => {
      setLoading(true);
      const { data } = await axiosInstance.get<OrdersResponse>(`/admin/orders?tab=${nextTab}&limit=${limit}`);
      setOrders(data.data);
  ...
  useEffect(() => {
    // history.replaceState rather than router.replace: this keeps the tab linkable
    // without asking Next to re-render the dashboard's server component.
  ...
  const columns = useMemo<Column[]>(() => {
    ...
    const lateBy: Column = {
      ...
      label: "Late by",
      skeleton: "w-20",
      render: (order) => <Badge tone="rust">{lateLabel(daysLate(order, now))}</Badge>,
    };
    ...
    switch (tab) {
      case "overdue":
        return [orderColumn, lateBy, placed, customer, items, total, payment, status, actions];
      case "completed":
        return [orderColumn, dateColumn("closed", "Last update", "updatedAt"), customer, items, total, payment, status, actions];
      case "cancelled":
        return [orderColumn, dateColumn("closed", "Last update", "updatedAt"), customer, total, payment, status, actions];
      default:
        return [orderColumn, placed, customer, items, total, payment, status, actions];
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- handlers are stable for the lifetime of the component
  }, [tab, now]);
  ...
  return (
    <section className="mt-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-patch-ink-muted">Orders</p>
          <p className="mt-1 text-sm text-patch-ink-muted">
            Triage by bucket. Showing the {limit} most relevant per tab.
          </p>
        </div>
        <Link href="/admin/orders" className="text-xs text-patch-accent hover:underline">
          All orders
        </Link>
      </div>
      ...
      <div role="tabpanel" id={orderTabPanelId(tab)} aria-labelledby={`orders-tab-${tab}`} tabIndex={-1} className="outline-none">
                ...
                {loading ? (
                  ...
                  orders.map((order) => (
                    <tr key={order._id} className={tableRowClass}>
                      {columns.map((column) => (
                        <td key={column.key} className={cn(tableCellClass, column.align === "right" && "text-right")}>
                          {column.render(order)}
                        </td>
                      ))}
                    </tr>
        ...
        {/* Mobile: stacked cards. Order triage happens away from a desk, and an
            eight-column table cannot be read on a phone. */}
